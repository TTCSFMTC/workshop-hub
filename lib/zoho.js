...
// broken down further into the individual parts within a job type.
export async function createInvoice(orgId, { contactId, lineItems, reference, notes }) {
  const res = await booksFetch(orgId, "/invoices", {
    ...
    body: JSON.stringify({
      ...
      line_items: lineItems.map((l) => ({ name: l.name, rate: l.amount, quantity: 1 })),
...
// Same duplicate-name fallback as findOrCreateContact above, but for the
// vendor side (contact_type "vendor" rather than "customer") — used when
// filing a supplier's purchase invoice as a Zoho Books Bill, not a sales
// invoice to a customer.
export async function findOrCreateVendorContact(orgId, { name, phone, email }) {
  const created = await booksFetch(orgId, "/contacts", {
    method: "POST",
    body: JSON.stringify({ contact_name: name, contact_type: "vendor", mobile: phone || "", email: email || "" }),
  });
  if (created.ok) return created.data.contact.contact_id;

  const isDuplicate = created.status === 400 && /already exists|duplicate/i.test(created.data.message || "");
  if (!isDuplicate) throw new Error(`Zoho vendor contact create failed: ${JSON.stringify(created.data)}`);

  const found = await booksFetch(orgId, `/contacts?contact_name=${encodeURIComponent(name)}`);
  const match = found.data.contacts?.find((c) => c.contact_name === name);
  if (!match) throw new Error(`Zoho reported "${name}" as a duplicate vendor contact but it couldn't be found`);
  return match.contact_id;
}

// One line item per invoice line as extracted from the supplier's PDF —
// filed as accounts payable (a Bill), not an Expense or Purchase Order,
// since this is money already owed for goods already received.
export async function createBill(orgId, { vendorId, billNumber, date, dueDate, lineItems, notes }) {
  const res = await booksFetch(orgId, "/bills", {
    method: "POST",
    body: JSON.stringify({
      vendor_id: vendorId,
      ...
      line_items: lineItems.map((l) => ({ name: l.description, rate: l.unitPrice ?? l.amount, quantity: l.quantity || 1 })),
      notes: notes || "",
    }),
  });
  if (!res.ok) throw new Error(`Zoho bill create failed: ${JSON.stringify(res.data)}`);
  return res.data.bill; // includes bill_id, bill_number, etc. — Bills have no shareable bill_url like invoices do
}
