...
export async function createInvoice(orgId, { contactId, lineItems, reference, notes }) {
  ...
  // The API's own invoice_url points at the customer-facing secure payment
  // portal (zohosecurepay.<dc>), not anywhere staff can view/manage the
  // invoice — so the button links into the Zoho Books app itself instead.
  invoice.invoice_url = `https://books.zoho.${DC}/app/${orgId}#/invoices/${invoice.invoice_id}`;
  return invoice; // includes invoice_id, invoice_number, invoice_url, etc.
}
