...
export async function uploadFileAndShare({ name, mimeType, buffer }) {
  ...
  const url = await makeShareable(id);
  return { id, url };
}

// Pulls a file's raw bytes back down — used to merge an already-generated
// PDF (the technical write-up) into another document server-side, rather
...
// Used when a file is being auto-regenerated (e.g. the technical write-up,
// re-uploaded on every save) so the folder doesn't accumulate a new copy
// every time — best-effort, a delete failure shouldn't block the fresh
...
export async function deleteFile(fileId) {
  ...
  if (!res.ok && res.status !== 404) {
    console.error(`Google Drive delete failed for ${fileId}: ${res.status} ${await res.text()}`);
  }
}
