...
const QUOTE_EXTRACTION_INSTRUCTIONS = `The material below relates to a vehicle repair job — it might be a diagnosis with parts/labour costs, a formatted quote, a reference price table, a numbered parts list, a rough breakdown (AI-drafted text, or a scanned/typed PDF), or a warranty/insurance company's schedule or booking confirmation (customer and vehicle details, possibly with no pricing at all). Pull out whatever's present into a structured quote.
...

{
  ...
  "customer_address": string or null (the customer's/policyholder's postal address, if given — combine multiple address lines into one string),
...
- If no customer name/vehicle/address is mentioned, use null rather than guessing.
...
- A document with no pricing at all (e.g. just a warranty schedule with customer/vehicle details) is still useful — return an empty line_items array in that case rather than treating it as a failure, since the customer/vehicle/address fields are still worth extracting on their own.
