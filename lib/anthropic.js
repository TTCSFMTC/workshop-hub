...
export async function extractQuoteFromScript({ scriptText }) {
  const prompt = `The text below is a repair quote drafted in a conversation with an AI assistant (Claude or ChatGPT) for a vehicle repair workshop — it might be a diagnosis with parts and labour costs, a formatted quote, or just a rough breakdown. Pull out a structured quote from it.
...
Rules:
- unit_price is always the price PER UNIT, EXCLUDING VAT (net) — if the script only gives VAT-inclusive or lump-sum prices, back-calculate the net unit price yourself.
...
- If no customer name/vehicle is mentioned in the text, use null rather than guessing.
- If nothing resembling a priced quote is found, return line_items as an empty array.
