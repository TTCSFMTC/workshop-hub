...
export async function sendBookingRequestNotification({ name, business, phone, reg, date, requirements, needsReview, isEmergency, otherDetails }) {
  if (!OFFICE_NOTIFICATION_EMAIL) return;
  ...
  const html = `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a1a;">
      ...
      <p><strong>${name || "Customer"}</strong> — ${phone || "no phone given"}${reg ? ` — ${reg}` : ""}</p>
