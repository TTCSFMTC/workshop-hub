...
export async function sendBookingRequestNotification({ name, business, phone, reg, date, requirements, needsReview }) {
  if (!OFFICE_NOTIFICATION_EMAIL) return;
  const html = `
    <div style="font-family: -apple-system, Helvetica, Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a1a;">
      <h2 style="margin-bottom: 4px;">New booking request${needsReview ? " — worth a closer look" : ""}</h2>
  ...
  return sendEmail({
    from: `Workshop Hub <noreply@${SEND_DOMAIN}>`,
    to: OFFICE_NOTIFICATION_EMAIL,
    subject: `New booking request: ${name || "Customer"}${reg ? ` (${reg})` : ""} — ${date}`,
    html,
  });
}
