...
// Renders the vehicle handover/collection sign-off as a one-page PDF — the
// customer's legal acknowledgement, taken at collection, that they've
// inspected the vehicle and are happy with the work and its condition.
// Distinct from the drop-off confirmation above (that's taken on arrival,
// before any work is done) and from the technical write-up's hand-signed
// copies (that's an explanation of findings, not a receipt of the vehicle).
export async function generateHandoverPdf({
  customerName, reg, vehicleModel, jobTypeName, signatureName, signatureDataUrl, confirmedAt,
}) {
  const doc = await PDFDocument.create();
  const page = doc.addPage([595, 842]); // A4
  const font = await doc.embedFont(StandardFonts.Helvetica);
  ...
  draw("Vehicle Handover Confirmation", { size: 18, useFont: bold, gap: 26 });
  draw(new Date(confirmedAt).toLocaleString("en-GB"), { size: 10, color: rgb(0.4, 0.4, 0.4), gap: 28 });
  ...
  if (reg) draw(reg);
  if (vehicleModel) draw(vehicleModel);
  if (jobTypeName) draw(`Work carried out: ${jobTypeName}`);
  y -= 14;

  draw("Customer acknowledgement", { size: 13, useFont: bold, gap: 20 });
