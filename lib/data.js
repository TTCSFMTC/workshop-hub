...
// ---- intake confirmations (vehicle drop-off record) ----
export async function insertIntakeConfirmation(intake) {
  const { error } = await supabase.from("intake_confirmations").insert({
    id: intake.id, booking_id: intake.bookingId, pre_scan_completed: intake.preScanCompleted,
    video_completed: intake.videoCompleted, signature: intake.signature, signature_name: intake.signatureName,
  });
  must(error);
}
