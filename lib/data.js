...
const bookingFromRow = (r) => ({
  ...
  createdAt: new Date(r.created_at).getTime(), googleEventId: r.google_event_id,
});
...
export async function fetchAll() {
  const [parts, jobTypes, bookings, jobCards, settings, priceHistory] = await Promise.all([
    fetchParts(), fetchJobTypes(), fetchBookings(), fetchJobCards(), fetchSettings(), fetchPriceHistory(),
  ]);
  return { parts, jobTypes, bookings, jobCards, settings, priceHistory };
}
