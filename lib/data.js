import { supabase } from "./supabaseClient";

// ============================================================
// Row <-> app-shape mapping
//
// The app (ported straight from the original prototype) works in camelCase
// objects, e.g. booking.customerName, jobType.bom, jobCard.jobStatus. These
// helpers are the only place that knows about the snake_case Postgres schema
// in supabase/schema.sql, so the rest of the app never has to change again
// if the schema changes.
// ============================================================

const partFromRow = (r) => ({ id: r.id, name: r.name, unit: r.unit, stock: Number(r.stock), costPrice: Number(r.cost_price), partNumber: r.part_number || "" });
const partToRow = (p) => ({ id: p.id, name: p.name, unit: p.unit, stock: p.stock, cost_price: p.costPrice, part_number: p.partNumber || null });

const brandFromRow = (r) => ({ id: r.id, name: r.name });

const priceHistoryFromRow = (r) => ({
  id: r.id, partId: r.part_id, price: Number(r.price), qty: r.qty === null ? null : Number(r.qty),
  supplier: r.supplier, recordedAt: r.recorded_at,
});

const stockBatchFromRow = (r) => ({
  id: r.id, partId: r.part_id, qtyOrdered: Number(r.qty_ordered), qtyRemaining: Number(r.qty_remaining),
  price: Number(r.price), supplier: r.supplier || "", status: r.status,
  orderedAt: r.ordered_at, deliveredAt: r.delivered_at, dueDate: r.due_date || null,
});

const bookingFromRow = (r) => ({
  id: r.id, business: r.business, customerName: r.customer_name, phone: r.phone, email: r.email || "", reg: r.reg,
  symptoms: r.symptoms, jobTypeId: r.job_type_id, date: r.date, days: Number(r.days ?? 1), pickupRequired: r.pickup_required,
  pickupAddress: r.pickup_address, postcode: r.postcode, distanceMiles: r.distance_miles === null ? null : Number(r.distance_miles),
  jobValue: Number(r.job_value), labourCost: Number(r.labour_cost), transportCost: Number(r.transport_cost),
  partsCostOverride: r.parts_cost_override === null || r.parts_cost_override === undefined ? null : Number(r.parts_cost_override),
  costsReviewed: r.costs_reviewed ?? false,
  createdAt: new Date(r.created_at).getTime(), googleEventId: r.google_event_id, vehicleModel: r.vehicle_model || "",
  completed: r.completed ?? false, reminderSent: r.reminder_sent ?? false, transportRequired: r.transport_required ?? false,
  completedAt: r.completed_at ? new Date(r.completed_at).getTime() : null, followupSent: r.followup_sent ?? false,
  reviewFollowupDone: r.review_followup_done ?? false,
  zohoInvoiceId: r.zoho_invoice_id || null, zohoInvoiceNumber: r.zoho_invoice_number || null, zohoInvoiceUrl: r.zoho_invoice_url || null,
  workshopCompleted: r.workshop_completed ?? false, workshopCompletedAt: r.workshop_completed_at ? new Date(r.workshop_completed_at).getTime() : null,
  arrived: r.arrived ?? false, arrivedAt: r.arrived_at ? new Date(r.arrived_at).getTime() : null,
  noShow: r.no_show ?? false, noShowAt: r.no_show_at ? new Date(r.no_show_at).getTime() : null,
  provisional: r.provisional ?? false,
  transportConfirmed: r.transport_confirmed,
  transportCollected: r.transport_collected ?? false, transportDelivered: r.transport_delivered ?? false,
  transportCustomerPaid: r.transport_customer_paid ?? false,
  paymentMethod: r.payment_method || "",
  notifyIfEarlierSlot: r.notify_if_earlier_slot ?? false,
  customerCancelled: r.customer_cancelled ?? false,
  customerCancelledAt: r.customer_cancelled_at ? new Date(r.customer_cancelled_at).getTime() : null,
  cancellationFilled: r.cancellation_filled ?? false,
  cancellationOfferedTo: r.cancellation_offered_to || null,
  cancellationOfferedAt: r.cancellation_offered_at ? new Date(r.cancellation_offered_at).getTime() : null,
});
const bookingToRow = (b) => ({
  id: b.id, business: b.business, customer_name: b.customerName, phone: b.phone, email: b.email || "", reg: b.reg,
  symptoms: b.symptoms, job_type_id: b.jobTypeId, date: b.date, days: b.days ?? 1, pickup_required: b.pickupRequired,
  pickup_address: b.pickupAddress, postcode: b.postcode, distance_miles: b.distanceMiles,
  job_value: b.jobValue, labour_cost: b.labourCost, transport_cost: b.transportCost,
  parts_cost_override: b.partsCostOverride ?? null, costs_reviewed: b.costsReviewed ?? false,
  google_event_id: b.googleEventId, vehicle_model: b.vehicleModel || null, completed: b.completed ?? false,
  reminder_sent: b.reminderSent ?? false, transport_required: b.transportRequired ?? false,
  completed_at: b.completedAt ? new Date(b.completedAt).toISOString() : null, followup_sent: b.followupSent ?? false,
  review_followup_done: b.reviewFollowupDone ?? false,
  zoho_invoice_id: b.zohoInvoiceId || null, zoho_invoice_number: b.zohoInvoiceNumber || null, zoho_invoice_url: b.zohoInvoiceUrl || null,
  workshop_completed: b.workshopCompleted ?? false, workshop_completed_at: b.workshopCompletedAt ? new Date(b.workshopCompletedAt).toISOString() : null,
  arrived: b.arrived ?? false, arrived_at: b.arrivedAt ? new Date(b.arrivedAt).toISOString() : null,
  no_show: b.noShow ?? false, no_show_at: b.noShowAt ? new Date(b.noShowAt).toISOString() : null,
  provisional: b.provisional ?? false,
  transport_confirmed: b.transportConfirmed ?? null,
  transport_collected: b.transportCollected ?? false, transport_delivered: b.transportDelivered ?? false,
  transport_customer_paid: b.transportCustomerPaid ?? false,
  payment_method: b.paymentMethod || null,
  notify_if_earlier_slot: b.notifyIfEarlierSlot ?? false,
  customer_cancelled: b.customerCancelled ?? false,
  customer_cancelled_at: b.customerCancelledAt ? new Date(b.customerCancelledAt).toISOString() : null,
  cancellation_filled: b.cancellationFilled ?? false,
  cancellation_offered_to: b.cancellationOfferedTo || null,
  cancellation_offered_at: b.cancellationOfferedAt ? new Date(b.cancellationOfferedAt).toISOString() : null,
});

const settingsFromRow = (r) => ({
  workshopPostcode: r.workshop_postcode, vatRegistered: r.vat_registered,
  collectionInfoUrl: r.collection_info_url, transportCompanies: r.transport_companies,
  transportContactName: r.transport_contact_name || "Paul", transportContactPhone: r.transport_contact_phone || "",
  transportContactEmail: r.transport_contact_email || "",
  monthlyTarget: r.monthly_target == null ? 40000 : Number(r.monthly_target),
  nonProductivesCost: r.non_productives_cost == null ? 5000 : Number(r.non_productives_cost),
  workingDaysPerMonth: r.working_days_per_month == null ? 25 : Number(r.working_days_per_month),
});
const settingsToRow = (s) => ({
  workshop_postcode: s.workshopPostcode, vat_registered: s.vatRegistered,
  collection_info_url: s.collectionInfoUrl, transport_companies: s.transportCompanies,
  transport_contact_name: s.transportContactName || "Paul", transport_contact_phone: s.transportContactPhone || "",
  transport_contact_email: s.transportContactEmail || "",
  monthly_target: s.monthlyTarget ?? 40000,
  non_productives_cost: s.nonProductivesCost ?? 5000,
  working_days_per_month: s.workingDaysPerMonth ?? 25,
});

const jobCardFromRow = (r) => ({
  id: r.id, bookingId: r.booking_id, business: r.business, createdAt: new Date(r.created_at).getTime(),
  dateIn: r.date_in || "", dateOut: r.date_out || "", technician: r.technician,
  make: r.make, model: r.model, reg: r.reg, vin: r.vin, transmission: r.transmission, drive: r.drive,
  mileageIn: r.mileage_in, mileageOut: r.mileage_out,
  customerName: r.customer_name, contact: r.contact, email: r.email,
  jobStatus: r.job_status, authRefNotes: r.auth_ref_notes, symptoms: r.symptoms,
  technicianInterpretation: r.technician_interpretation, preDiagnostic: r.pre_diagnostic,
  diagnosisFindings: r.diagnosis_findings, postDiagnostic: r.post_diagnostic, postChecks: r.post_checks,
  videoLog: r.video_log, signature: r.signature, signatureName: r.signature_name,
  signatureDate: r.signature_date, locked: r.locked, completionVideoOverridden: r.completion_video_overridden,
  technicalWriteupUrl: r.technical_writeup_url || null, technicalWriteupUpdatedAt: r.technical_writeup_updated_at || null,
  requiredBy: r.required_by || "",
  handoverPdfUrl: r.handover_pdf_url || null,
});
const jobCardToRow = (c) => ({
  id: c.id, booking_id: c.bookingId, business: c.business,
  date_in: c.dateIn || null, date_out: c.dateOut || null, technician: c.technician,
  make: c.make, model: c.model, reg: c.reg, vin: c.vin, transmission: c.transmission, drive: c.drive,
  mileage_in: c.mileageIn, mileage_out: c.mileageOut,
  customer_name: c.customerName, contact: c.contact, email: c.email,
  job_status: c.jobStatus, auth_ref_notes: c.authRefNotes, symptoms: c.symptoms,
  technician_interpretation: c.technicianInterpretation, pre_diagnostic: c.preDiagnostic,
  diagnosis_findings: c.diagnosisFindings, post_diagnostic: c.postDiagnostic, post_checks: c.postChecks,
  video_log: c.videoLog, signature: c.signature, signature_name: c.signatureName,
  signature_date: c.signatureDate, locked: c.locked, completion_video_overridden: c.completionVideoOverridden ?? false,
  required_by: c.requiredBy || null,
  handover_pdf_url: c.handoverPdfUrl || null,
});

const jobApprovalFromRow = (r) => ({
  id: r.id, jobCardId: r.job_card_id, bookingId: r.booking_id, token: r.token,
  description: r.description, aiWriteup: r.ai_writeup,
  price: r.price === null ? null : Number(r.price), inStock: r.in_stock, status: r.status,
  createdAt: r.created_at, sentAt: r.sent_at, respondedAt: r.responded_at,
  customerSignature: r.customer_signature, customerSignatureName: r.customer_signature_name,
});

const supplierFromRow = (r) => ({ id: r.id, name: r.name, contactEmail: r.contact_email || "", contactName: r.contact_name || "", createdAt: r.created_at });

const supplierInvoiceFromRow = (r) => ({
  id: r.id, supplierId: r.supplier_id || null, stockBatchId: r.stock_batch_id || null, business: r.business,
  pdfUrl: r.pdf_url, pdfDriveFileId: r.pdf_drive_file_id || null,
  vendorNameRaw: r.vendor_name_raw || "", invoiceNumber: r.invoice_number || "",
  invoiceDate: r.invoice_date, dueDate: r.due_date,
  subtotal: r.subtotal === null ? null : Number(r.subtotal), tax: r.tax === null ? null : Number(r.tax), total: Number(r.total),
  lineItems: r.line_items || [], status: r.status,
  uploadedAt: r.uploaded_at, confirmedAt: r.confirmed_at,
});

const quoteFromRow = (r) => ({
  id: r.id, business: r.business,
  customerName: r.customer_name || "", customerEmail: r.customer_email || "", customerPhone: r.customer_phone || "",
  customerAddress: r.customer_address || "",
  vehicleDescription: r.vehicle_description || "", sourceScript: r.source_script || "",
  sourcePdfUrl: r.source_pdf_url || null,
  lineItems: r.line_items || [],
  subtotal: Number(r.subtotal), vatRate: Number(r.vat_rate), vat: Number(r.vat), total: Number(r.total),
  notes: r.notes || "", status: r.status,
  zohoEstimateId: r.zoho_estimate_id || null, zohoEstimateNumber: r.zoho_estimate_number || null, zohoEstimateUrl: r.zoho_estimate_url || null,
  createdAt: r.created_at, postedAt: r.posted_at,
});

function must(error) {
  if (error) throw error;
}

// ---- fetch ----
export async function fetchParts() {
  const { data, error } = await supabase.from("parts").select("*").order("name");
  must(error);
  return data.map(partFromRow);
}

export async function fetchJobTypes() {
  const [{ data: jt, error: e1 }, { data: jtp, error: e2 }] = await Promise.all([
    supabase.from("job_types").select("*").order("name"),
    supabase.from("job_type_parts").select("*"),
  ]);
  must(e1); must(e2);
  return jt.map((row) => ({
    id: row.id,
    name: row.name,
    color: row.color,
    brandId: row.brand_id || null,
    standardPrice: row.standard_price === null || row.standard_price === undefined ? null : Number(row.standard_price),
    publicBookable: row.public_bookable ?? false,
    bom: jtp.filter((l) => l.job_type_id === row.id).map((l) => ({ partId: l.part_id, qty: Number(l.qty) })),
  }));
}

export async function fetchBrands() {
  const { data, error } = await supabase.from("brands").select("*").order("sort_order");
  must(error);
  return data.map(brandFromRow);
}

export async function fetchAuditLog() {
  const { data, error } = await supabase.from("audit_log").select("*").order("created_at", { ascending: false });
  must(error);
  return data.map((r) => ({ id: r.id, summary: r.summary, reason: r.reason, createdAt: r.created_at }));
}
export async function insertAuditLog(entry) {
  const { error } = await supabase.from("audit_log").insert({ id: entry.id, summary: entry.summary, reason: entry.reason, created_at: entry.createdAt });
  must(error);
}

export async function fetchHolidays() {
  const { data, error } = await supabase.from("holidays").select("*").order("date_from");
  must(error);
  return data.map((r) => ({ id: r.id, name: r.name, dateFrom: r.date_from, dateTo: r.date_to }));
}

export async function fetchSuppliers() {
  const { data, error } = await supabase.from("suppliers").select("*").order("name");
  must(error);
  return data.map(supplierFromRow);
}

export async function fetchSupplierInvoices() {
  const { data, error } = await supabase.from("supplier_invoices").select("*").order("uploaded_at", { ascending: false });
  must(error);
  return data.map(supplierInvoiceFromRow);
}

export async function fetchQuotes() {
  const { data, error } = await supabase.from("quotes").select("*").order("created_at", { ascending: false });
  must(error);
  return data.map(quoteFromRow);
}

export async function fetchBonusRates() {
  const { data, error } = await supabase.from("bonus_rates").select("*").order("sort_order");
  must(error);
  return data.map((r) => ({ id: r.id, name: r.name, rate: Number(r.rate), jobTypeIds: r.job_type_ids || [] }));
}

export async function fetchStaffWages() {
  const { data, error } = await supabase.from("staff_wages").select("*").order("month");
  must(error);
  return data.map((r) => ({
    id: r.id, name: r.name, month: r.month, basic: Number(r.basic),
    weekendFullDays: Number(r.weekend_full_days), weekendHalfDays: Number(r.weekend_half_days),
    bonusCounts: r.bonus_counts || {},
  }));
}

export async function fetchBookings() {
  const [{ data, error }, { data: extras, error: e2 }, { data: extraParts, error: e3 }, { data: jobTypePrices, error: e4 }, { data: bomQtyOverrides, error: e5 }, extraCostsResult, bomCostOverridesResult] = await Promise.all([
    supabase.from("bookings").select("*").order("date"),
    supabase.from("booking_job_types").select("*"),
    supabase.from("booking_extra_parts").select("*"),
    supabase.from("booking_job_type_prices").select("*"),
    supabase.from("booking_bom_qty_overrides").select("*"),
    // Not `must()`-ed like the others — these tables are much newer, so a
    // missing-migration gap (or any other hiccup fetching them) shouldn't be
    // able to blank out every booking in the app the way bundling
    // suppliers/supplier_invoices into this same fetch once did.
    supabase.from("booking_extra_costs").select("*").then((r) => r, (e) => ({ data: null, error: e })),
    supabase.from("booking_bom_cost_overrides").select("*").then((r) => r, (e) => ({ data: null, error: e })),
  ]);
  must(error); must(e2); must(e3); must(e4); must(e5);
  if (extraCostsResult.error) console.error("fetchBookings: booking_extra_costs failed, continuing without it", extraCostsResult.error);
  if (bomCostOverridesResult.error) console.error("fetchBookings: booking_bom_cost_overrides failed, continuing without it", bomCostOverridesResult.error);
  const extraCosts = extraCostsResult.data || [];
  const bomCostOverrides = bomCostOverridesResult.data || [];
  return data.map((row) => ({
    ...bookingFromRow(row),
    extraJobTypeIds: extras.filter((l) => l.booking_id === row.id).map((l) => l.job_type_id),
    extraParts: extraParts.filter((l) => l.booking_id === row.id).map((l) => ({ partId: l.part_id, qty: Number(l.qty) })),
    jobTypePrices: jobTypePrices.filter((l) => l.booking_id === row.id).map((l) => ({ jobTypeId: l.job_type_id, price: Number(l.price) })),
    bomQtyOverrides: bomQtyOverrides.filter((l) => l.booking_id === row.id).map((l) => ({ partId: l.part_id, qty: Number(l.qty) })),
    bomCostOverrides: bomCostOverrides.filter((l) => l.booking_id === row.id).map((l) => ({ partId: l.part_id, cost: Number(l.cost) })),
    extraCosts: extraCosts
      .filter((l) => l.booking_id === row.id)
      .map((l) => ({ id: l.id, description: l.description, amount: Number(l.amount), createdAt: l.created_at }))
      .sort((a, b) => (a.createdAt < b.createdAt ? -1 : 1)),
  }));
}

export async function fetchPriceHistory() {
  const { data, error } = await supabase.from("part_price_history").select("*").order("recorded_at");
  must(error);
  return data.map(priceHistoryFromRow);
}

export async function fetchStockBatches() {
  const { data, error } = await supabase.from("stock_batches").select("*").order("ordered_at");
  must(error);
  return data.map(stockBatchFromRow);
}

export async function fetchJobCards() {
  const { data, error } = await supabase.from("job_cards").select("*").order("created_at", { ascending: false });
  must(error);
  return data.map(jobCardFromRow);
}

export async function fetchJobApprovals() {
  const { data, error } = await supabase.from("job_approvals").select("*").order("created_at", { ascending: false });
  must(error);
  return data.map(jobApprovalFromRow);
}

export async function fetchSettings() {
  const { data, error } = await supabase.from("settings").select("*").eq("id", true).maybeSingle();
  must(error);
  return data ? settingsFromRow(data) : null;
}

export async function fetchAll() {
  const [parts, jobTypes, bookings, jobCards, jobApprovals, settings, priceHistory, stockBatches, brands, holidays, bonusRates, staffWages, fixedCosts, auditLog, plSnapshots] = await Promise.all([
    fetchParts(), fetchJobTypes(), fetchBookings(), fetchJobCards(), fetchJobApprovals(), fetchSettings(), fetchPriceHistory(), fetchStockBatches(), fetchBrands(), fetchHolidays(), fetchBonusRates(), fetchStaffWages(), fetchFixedCosts(), fetchAuditLog(), fetchPLSnapshots(),
  ]);
  return { parts, jobTypes, bookings, jobCards, jobApprovals, settings, priceHistory, stockBatches, brands, holidays, bonusRates, staffWages, fixedCosts, auditLog, plSnapshots };
}

// ---- parts ----
export async function insertPart(part) {
  const { error } = await supabase.from("parts").insert(partToRow(part));
  must(error);
}
export async function updatePart(id, patch) {
  const row = {};
  if ("name" in patch) row.name = patch.name;
  if ("unit" in patch) row.unit = patch.unit;
  if ("stock" in patch) row.stock = patch.stock;
  if ("costPrice" in patch) row.cost_price = patch.costPrice;
  if ("partNumber" in patch) row.part_number = patch.partNumber || null;
  const { error } = await supabase.from("parts").update(row).eq("id", id);
  must(error);
}
export async function deletePart(id) {
  const { error } = await supabase.from("parts").delete().eq("id", id);
  must(error);
}

// ---- job types & recipe lines ----
export async function insertJobType(jobType) {
  const { error } = await supabase.from("job_types").insert({ id: jobType.id, name: jobType.name, brand_id: jobType.brandId || null });
  must(error);
}
export async function renameJobType(id, name) {
  const { error } = await supabase.from("job_types").update({ name }).eq("id", id);
  must(error);
}
export async function updateJobTypeColor(id, color) {
  const { error } = await supabase.from("job_types").update({ color }).eq("id", id);
  must(error);
}
export async function updateJobTypeBrand(id, brandId) {
  const { error } = await supabase.from("job_types").update({ brand_id: brandId }).eq("id", id);
  must(error);
}
export async function updateJobTypeStandardPrice(id, price) {
  const { error } = await supabase.from("job_types").update({ standard_price: price }).eq("id", id);
  must(error);
}
export async function updateJobTypePublicBookable(id, publicBookable) {
  const { error } = await supabase.from("job_types").update({ public_bookable: publicBookable }).eq("id", id);
  must(error);
}
export async function deleteJobType(id) {
  const { error } = await supabase.from("job_types").delete().eq("id", id);
  must(error);
}

// ---- brands ----
export async function insertBrand(brand) {
  const { error } = await supabase.from("brands").insert({ id: brand.id, name: brand.name });
  must(error);
}
export async function deleteBrand(id) {
  const { error } = await supabase.from("brands").delete().eq("id", id);
  must(error);
}
export async function renameBrand(id, name) {
  const { error } = await supabase.from("brands").update({ name }).eq("id", id);
  must(error);
}

// ---- holidays ----
export async function insertHoliday(holiday) {
  const { error } = await supabase.from("holidays").insert({ id: holiday.id, name: holiday.name, date_from: holiday.dateFrom, date_to: holiday.dateTo });
  must(error);
}
export async function deleteHoliday(id) {
  const { error } = await supabase.from("holidays").delete().eq("id", id);
  must(error);
}

// ---- suppliers ----
export async function insertSupplier(supplier) {
  const { error } = await supabase.from("suppliers").insert({
    id: supplier.id, name: supplier.name, contact_email: supplier.contactEmail || null, contact_name: supplier.contactName || null,
  });
  must(error);
}
export async function updateSupplier(id, patch) {
  const row = {};
  if (patch.name !== undefined) row.name = patch.name;
  if (patch.contactEmail !== undefined) row.contact_email = patch.contactEmail || null;
  if (patch.contactName !== undefined) row.contact_name = patch.contactName || null;
  const { error } = await supabase.from("suppliers").update(row).eq("id", id);
  must(error);
}
export async function deleteSupplier(id) {
  const { error } = await supabase.from("suppliers").delete().eq("id", id);
  must(error);
}

// ---- supplier invoices ----
// Row creation happens server-side (see app/api/office/supplier-invoices/scan)
// alongside the Drive upload + Claude extraction, not through this file —
// this is only for reading and editing invoices already on file.
export async function updateSupplierInvoice(id, patch) {
  const row = {};
  if (patch.supplierId !== undefined) row.supplier_id = patch.supplierId || null;
  if (patch.stockBatchId !== undefined) row.stock_batch_id = patch.stockBatchId || null;
  if (patch.business !== undefined) row.business = patch.business;
  if (patch.status !== undefined) row.status = patch.status;
  if (patch.confirmedAt !== undefined) row.confirmed_at = patch.confirmedAt;
  const { error } = await supabase.from("supplier_invoices").update(row).eq("id", id);
  must(error);
}
export async function deleteSupplierInvoice(id) {
  const { error } = await supabase.from("supplier_invoices").delete().eq("id", id);
  must(error);
}

// ---- quotes ----
// Row creation happens server-side (see app/api/office/quotes/generate)
// alongside the Claude extraction, not through this file — this is only
// for reading and editing quotes already on file.
export async function updateQuote(id, patch) {
  const row = {};
  if (patch.business !== undefined) row.business = patch.business;
  if (patch.customerName !== undefined) row.customer_name = patch.customerName || null;
  if (patch.customerEmail !== undefined) row.customer_email = patch.customerEmail || null;
  if (patch.customerPhone !== undefined) row.customer_phone = patch.customerPhone || null;
  if (patch.customerAddress !== undefined) row.customer_address = patch.customerAddress || null;
  if (patch.vehicleDescription !== undefined) row.vehicle_description = patch.vehicleDescription || null;
  if (patch.lineItems !== undefined) row.line_items = patch.lineItems;
  if (patch.subtotal !== undefined) row.subtotal = patch.subtotal;
  if (patch.vatRate !== undefined) row.vat_rate = patch.vatRate;
  if (patch.vat !== undefined) row.vat = patch.vat;
  if (patch.total !== undefined) row.total = patch.total;
  if (patch.notes !== undefined) row.notes = patch.notes || null;
  if (patch.status !== undefined) row.status = patch.status;
  const { error } = await supabase.from("quotes").update(row).eq("id", id);
  must(error);
}
export async function deleteQuote(id) {
  const { error } = await supabase.from("quotes").delete().eq("id", id);
  must(error);
}

// ---- bonus rates ----
export async function insertBonusRate(rate) {
  const { error } = await supabase.from("bonus_rates").insert({ id: rate.id, name: rate.name, rate: rate.rate, job_type_ids: rate.jobTypeIds || [] });
  must(error);
}
export async function updateBonusRate(id, rate) {
  const { error } = await supabase.from("bonus_rates").update({ rate }).eq("id", id);
  must(error);
}
export async function updateBonusRateJobTypes(id, jobTypeIds) {
  const { error } = await supabase.from("bonus_rates").update({ job_type_ids: jobTypeIds }).eq("id", id);
  must(error);
}
export async function deleteBonusRate(id) {
  const { error } = await supabase.from("bonus_rates").delete().eq("id", id);
  must(error);
}

// ---- staff wages ----
export async function upsertStaffWage(wage) {
  const { error } = await supabase.from("staff_wages").upsert({
    id: wage.id, name: wage.name, month: wage.month, basic: wage.basic,
    weekend_full_days: wage.weekendFullDays, weekend_half_days: wage.weekendHalfDays,
    bonus_counts: wage.bonusCounts,
  });
  must(error);
}
export async function deleteStaffWage(id) {
  const { error } = await supabase.from("staff_wages").delete().eq("id", id);
  must(error);
}
export async function addBomLine(jobTypeId, partId, qty) {
  const { error } = await supabase.from("job_type_parts").upsert({ job_type_id: jobTypeId, part_id: partId, qty });
  must(error);
}
export async function updateBomLine(jobTypeId, partId, qty) {
  const { error } = await supabase.from("job_type_parts").update({ qty }).eq("job_type_id", jobTypeId).eq("part_id", partId);
  must(error);
}
export async function removeBomLine(jobTypeId, partId) {
  const { error } = await supabase.from("job_type_parts").delete().eq("job_type_id", jobTypeId).eq("part_id", partId);
  must(error);
}

// ---- settings (single row) ----
export async function saveSettings(settings) {
  const { error } = await supabase.from("settings").update(settingsToRow(settings)).eq("id", true);
  must(error);
}

// ---- bookings ----
export async function insertBooking(booking) {
  const { error } = await supabase.from("bookings").insert(bookingToRow(booking));
  must(error);
}
// Fields the app stores as epoch-ms numbers but the DB stores as timestamptz
// — patches touching these must be converted to ISO strings, or PostgREST
// rejects the whole row update with a 400 (date/time field value out of range).
const BOOKING_ROW_DATE_KEYS = new Set(["completedAt", "workshopCompletedAt", "arrivedAt", "noShowAt", "customerCancelledAt", "cancellationOfferedAt"]);

export async function updateBookingRow(id, patch) {
  const row = {};
  const map = {
    customerName: "customer_name", jobTypeId: "job_type_id", pickupRequired: "pickup_required",
    pickupAddress: "pickup_address", distanceMiles: "distance_miles", jobValue: "job_value",
    labourCost: "labour_cost", transportCost: "transport_cost", googleEventId: "google_event_id",
    partsCostOverride: "parts_cost_override", costsReviewed: "costs_reviewed",
    vehicleModel: "vehicle_model", reminderSent: "reminder_sent", transportRequired: "transport_required",
    completedAt: "completed_at", followupSent: "followup_sent", reviewFollowupDone: "review_followup_done",
    zohoInvoiceId: "zoho_invoice_id", zohoInvoiceNumber: "zoho_invoice_number", zohoInvoiceUrl: "zoho_invoice_url",
    workshopCompleted: "workshop_completed", workshopCompletedAt: "workshop_completed_at",
    arrived: "arrived", arrivedAt: "arrived_at", noShow: "no_show", noShowAt: "no_show_at", provisional: "provisional", transportConfirmed: "transport_confirmed",
    transportCollected: "transport_collected", transportDelivered: "transport_delivered", transportCustomerPaid: "transport_customer_paid",
    paymentMethod: "payment_method",
    notifyIfEarlierSlot: "notify_if_earlier_slot", customerCancelled: "customer_cancelled", customerCancelledAt: "customer_cancelled_at",
    cancellationFilled: "cancellation_filled", cancellationOfferedTo: "cancellation_offered_to", cancellationOfferedAt: "cancellation_offered_at",
  };
  for (const [key, val] of Object.entries(patch)) {
    row[map[key] || key] = BOOKING_ROW_DATE_KEYS.has(key) && val != null ? new Date(val).toISOString() : val;
  }
  const { error } = await supabase.from("bookings").update(row).eq("id", id);
  must(error);
}
export async function deleteBookingRow(id) {
  const { error } = await supabase.from("bookings").delete().eq("id", id);
  must(error);
}

// ---- booking extra job types ----
export async function addBookingJobType(bookingId, jobTypeId) {
  const { error } = await supabase.from("booking_job_types").upsert({ booking_id: bookingId, job_type_id: jobTypeId });
  must(error);
}
export async function removeBookingJobType(bookingId, jobTypeId) {
  const { error } = await supabase.from("booking_job_types").delete().eq("booking_id", bookingId).eq("job_type_id", jobTypeId);
  must(error);
}

// ---- booking extra individual parts ----
export async function setBookingExtraPart(bookingId, partId, qty) {
  const { error } = await supabase.from("booking_extra_parts").upsert({ booking_id: bookingId, part_id: partId, qty });
  must(error);
}
export async function removeBookingExtraPart(bookingId, partId) {
  const { error } = await supabase.from("booking_extra_parts").delete().eq("booking_id", bookingId).eq("part_id", partId);
  must(error);
}

// ---- per-booking BOM quantity overrides (job-type parts that vary per vehicle) ----
export async function setBookingBomQtyOverride(bookingId, partId, qty) {
  const { error } = await supabase.from("booking_bom_qty_overrides").upsert({ booking_id: bookingId, part_id: partId, qty });
  must(error);
}
export async function removeBookingBomQtyOverride(bookingId, partId) {
  const { error } = await supabase.from("booking_bom_qty_overrides").delete().eq("booking_id", bookingId).eq("part_id", partId);
  must(error);
}

// ---- per-booking BOM unit cost overrides (a specific job paid a different
// price for a part than its current derived stock cost, or the part's
// never been costed at all) ----
export async function setBookingBomCostOverride(bookingId, partId, cost) {
  const { error } = await supabase.from("booking_bom_cost_overrides").upsert({ booking_id: bookingId, part_id: partId, cost });
  must(error);
}
export async function removeBookingBomCostOverride(bookingId, partId) {
  const { error } = await supabase.from("booking_bom_cost_overrides").delete().eq("booking_id", bookingId).eq("part_id", partId);
  must(error);
}

// ---- itemised ad-hoc extra costs (a part bought outside the recipe, a
// call-out, anything not already captured by Parts/Labour/Transport) ----
export async function insertBookingExtraCost(entry) {
  const { error } = await supabase.from("booking_extra_costs").insert({
    id: entry.id, booking_id: entry.bookingId, description: entry.description, amount: entry.amount, created_at: entry.createdAt,
  });
  must(error);
}
export async function removeBookingExtraCost(id) {
  const { error } = await supabase.from("booking_extra_costs").delete().eq("id", id);
  must(error);
}

// ---- per-job-type price breakdown ----
export async function setBookingJobTypePrice(bookingId, jobTypeId, price) {
  const { error } = await supabase.from("booking_job_type_prices").upsert({ booking_id: bookingId, job_type_id: jobTypeId, price });
  must(error);
}
export async function removeBookingJobTypePrice(bookingId, jobTypeId) {
  const { error } = await supabase.from("booking_job_type_prices").delete().eq("booking_id", bookingId).eq("job_type_id", jobTypeId);
  must(error);
}

// ---- part price history ----
export async function insertPriceHistory(entry) {
  const { error } = await supabase.from("part_price_history").insert({
    id: entry.id, part_id: entry.partId, price: entry.price, qty: entry.qty, supplier: entry.supplier || null, recorded_at: entry.recordedAt,
  });
  must(error);
}
export async function deletePriceHistory(id) {
  const { error } = await supabase.from("part_price_history").delete().eq("id", id);
  must(error);
}
export async function updatePriceHistorySupplier(id, supplier) {
  const { error } = await supabase.from("part_price_history").update({ supplier: supplier || null }).eq("id", id);
  must(error);
}

// ---- stock batches (FIFO-priced ordering + delivery) ----
export async function insertStockBatch(batch) {
  const { error } = await supabase.from("stock_batches").insert({
    id: batch.id, part_id: batch.partId, qty_ordered: batch.qtyOrdered, qty_remaining: batch.qtyRemaining,
    price: batch.price, supplier: batch.supplier || null, status: batch.status,
    ordered_at: batch.orderedAt, delivered_at: batch.deliveredAt || null, due_date: batch.dueDate || null,
  });
  must(error);
}
export async function updateStockBatchQtyRemaining(id, qtyRemaining) {
  const { error } = await supabase.from("stock_batches").update({ qty_remaining: qtyRemaining }).eq("id", id);
  must(error);
}
// Amends a pending order still awaiting delivery (wrong qty/price typed in,
// supplier changed their price, or not enough was actually available) —
// qty_ordered and qty_remaining are kept in lockstep since nothing's been
// delivered from this batch yet, so there's no partial-consumption to preserve.
export async function updateStockBatch(id, patch) {
  const row = {};
  if (patch.qty !== undefined) { row.qty_ordered = patch.qty; row.qty_remaining = patch.qty; }
  if (patch.price !== undefined) row.price = patch.price;
  if (patch.supplier !== undefined) row.supplier = patch.supplier || null;
  if (patch.dueDate !== undefined) row.due_date = patch.dueDate || null;
  const { error } = await supabase.from("stock_batches").update(row).eq("id", id);
  must(error);
}
export async function markStockBatchDelivered(id, deliveredAt) {
  const { error } = await supabase.from("stock_batches").update({ status: "delivered", delivered_at: deliveredAt }).eq("id", id);
  must(error);
}
export async function deleteStockBatch(id) {
  const { error } = await supabase.from("stock_batches").delete().eq("id", id);
  must(error);
}
export async function updateStockBatchSupplier(id, supplier) {
  const { error } = await supabase.from("stock_batches").update({ supplier: supplier || null }).eq("id", id);
  must(error);
}

// ---- fixed costs ----
export async function fetchFixedCosts() {
  const { data, error } = await supabase.from("fixed_costs").select("*").order("sort_order");
  must(error);
  return data.map((r) => ({ id: r.id, name: r.name, amount: Number(r.amount) }));
}
export async function insertFixedCost(fc) {
  const { error } = await supabase.from("fixed_costs").insert({ id: fc.id, name: fc.name, amount: fc.amount });
  must(error);
}
export async function updateFixedCost(id, fields) {
  const patch = {};
  if (fields.name !== undefined) patch.name = fields.name;
  if (fields.amount !== undefined) patch.amount = fields.amount;
  const { error } = await supabase.from("fixed_costs").update(patch).eq("id", id);
  must(error);
}
export async function deleteFixedCost(id) {
  const { error } = await supabase.from("fixed_costs").delete().eq("id", id);
  must(error);
}

// ---- monthly Profit & Loss snapshots — freezing a month saves its whole
// computed P&L (revenue, VAT, costs, wages, fixed costs, net profit) so it
// stays exactly as it was, even if bookings/wages get edited afterwards.
// Not `must()`-ed on fetch — this table's brand new, so a missing-migration
// gap shouldn't blank out the rest of the app the way a fatal fetch would.
export async function fetchPLSnapshots() {
  const { data, error } = await supabase.from("pl_snapshots").select("*").order("month");
  if (error) { console.error("fetchPLSnapshots failed, continuing without it", error); return []; }
  return data.map((r) => ({ id: r.id, month: r.month, snapshot: r.snapshot, frozenAt: r.frozen_at }));
}
export async function upsertPLSnapshot(row) {
  const { error } = await supabase.from("pl_snapshots").upsert(
    { id: row.id, month: row.month, snapshot: row.snapshot, frozen_at: row.frozenAt },
    { onConflict: "month" }
  );
  must(error);
}
export async function deletePLSnapshot(id) {
  const { error } = await supabase.from("pl_snapshots").delete().eq("id", id);
  must(error);
}

// ---- job cards ----
export async function upsertJobCardRow(card) {
  const { error } = await supabase.from("job_cards").upsert(jobCardToRow(card));
  must(error);
}
export async function updateJobCardRow(id, patch) {
  const map = {
    bookingId: "booking_id", dateIn: "date_in", dateOut: "date_out", mileageIn: "mileage_in",
    mileageOut: "mileage_out", customerName: "customer_name", jobStatus: "job_status",
    authRefNotes: "auth_ref_notes", technicianInterpretation: "technician_interpretation",
    preDiagnostic: "pre_diagnostic", diagnosisFindings: "diagnosis_findings",
    postDiagnostic: "post_diagnostic", postChecks: "post_checks", videoLog: "video_log",
    signatureName: "signature_name", signatureDate: "signature_date", completionVideoOverridden: "completion_video_overridden",
    requiredBy: "required_by", handoverPdfUrl: "handover_pdf_url",
  };
  const row = {};
  for (const [key, val] of Object.entries(patch)) row[map[key] || key] = val;
  const { error } = await supabase.from("job_cards").update(row).eq("id", id);
  must(error);
}
export async function deleteJobCardRow(id) {
  const { error } = await supabase.from("job_cards").delete().eq("id", id);
  must(error);
}

// ---- job approvals (distance customer approval reports) ----
export async function insertJobApproval(approval) {
  const { error } = await supabase.from("job_approvals").insert({
    id: approval.id, job_card_id: approval.jobCardId, booking_id: approval.bookingId,
    description: approval.description,
  });
  must(error);
}
const JOB_APPROVAL_DATE_KEYS = new Set(["sentAt", "respondedAt"]);

export async function updateJobApprovalRow(id, patch) {
  const map = {
    aiWriteup: "ai_writeup", inStock: "in_stock", sentAt: "sent_at", respondedAt: "responded_at",
    customerSignature: "customer_signature", customerSignatureName: "customer_signature_name",
  };
  const row = {};
  for (const [key, val] of Object.entries(patch)) {
    row[map[key] || key] = JOB_APPROVAL_DATE_KEYS.has(key) && val != null ? new Date(val).toISOString() : val;
  }
  const { error } = await supabase.from("job_approvals").update(row).eq("id", id);
  must(error);
}
export async function deleteJobApproval(id) {
  const { error } = await supabase.from("job_approvals").delete().eq("id", id);
  must(error);
}

// ---- realtime ----
// Subscribes to every change on `table` and calls `onChange` so the caller
// can re-fetch that slice. Data volumes here are small (single workshop),
// so "any change -> refetch the table" is simpler and more robust than
// diffing individual row events, at negligible cost.
export function subscribeTable(table, onChange) {
  const channel = supabase
    .channel(`${table}-changes`)
    .on("postgres_changes", { event: "*", schema: "public", table }, onChange)
    .subscribe();
  return () => supabase.removeChannel(channel);
}
